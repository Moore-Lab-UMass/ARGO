...
import { CompBiosample, GeneLinkingMethod } from "../../../../types";
...
interface BiosampleSelectProps {
    ...
    handleLinkageBiosampleSubmit: (method: GeneLinkingMethod, biosample: CompBiosample, include: boolean) => void
    extraRows: CompBiosample[];
...
export const LinkageBiosampleModal: React.FC<BiosampleSelectProps> = ({
    ...
    const [selected, setSelected] = useState<CompBiosample | null>(null);
    ...
    const rows: CompBiosample[] = useMemo(() => {
        ...
        const extraByName = new Map(
            extraRows.map((e) => [e.name, e])
        );

        const encodeNames = new Set(encodeWithRnaSeq.map((b) => b.name));

        //merge cellType into matching encode samples
        const matchingEncodeSamples = encodeWithRnaSeq
            .filter((b) => extraByName.has(b.name))
            .map((b) => ({
                ...b,
                cellType: extraByName.get(b.name)?.cellType,
            }));
    ...
    const handleSelectionChange = (samples: CompBiosample[]) => {
